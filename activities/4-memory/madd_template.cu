...
__global__ void kernel_1t1c(float *A, float *B, float *C, unsigned long WIDTH)
{
	...
	if (column < WIDTH)
	{
		for (int i = 0; i < WIDTH; i++)
		{
			C[column * WIDTH + i] = A[column * WIDTH + i] + B[column * WIDTH + i];
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	int size = 64;
	const dim3 threadsPerBlock(size, size);									  // Must not exceed 1024 (max thread per block)
	const dim3 blocksPerGrid(ceil(SQWIDTH / (size/1.0)), ceil(SQWIDTH / (size/1.0))); // Number of blocks that will be used
